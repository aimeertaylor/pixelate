...
# pixels) in the x and y direction, which is specified by num_pix_xy_K.
#
# Rationale: for a given dot matrix, it is easy to envision a target number of
# least-resolved large pixels of size K. We thus specifiy this target, then back
# calculate the dpp for pixel size k = 2, which is the smallest, most resolved,
# multi-dot pixel size. Note that some parameter combination are incompatible
# with the dot matrix.
#=================================================================================
compute_dpp_2 = function(num_pix_xy_K, # Specify the number of k = K pixels in x and y direction
                         K, scale, scale_factor, # Parameters used in compute_ddp
                         dot_matrix_dim){ # Dot matrix dimensions

  # Compute the dpp for k = K in the x and y direction
  # Use floor to allow for some that are partial at the ends
  dpp_K = ceiling(dot_matrix_dim/num_pix_xy_K)

  # Compute the dpp for k = 2 in the x and y direction
  if(scale == 'linear'){
    dpp_2 = sapply(floor(dpp_K/((2*scale_factor)^(K-2))), as.integer)
  } else if (scale == 'exponential'){
    dpp_2 = sapply(floor(dpp_K^(1/((2*scale_factor)^(K-2)))), as.integer)
  } else {
    stop('Scale must either be "linear" or "exponential"')
  }

  # Compatibility check
  if(any(dpp_2 < 2)){stop('Inputs (num_pix_xy_K, K, scale, scale_factor) are incompatible with the dot matrix.
                                  Consider a linear scale if not already, and/or reducing num_pix_xy_K, K, scale_factor.')}
...
#
# This function computes dots per pixel (dpp) for k = 3,...,K differently sized
# pixels and returns the dpps for all k = 1,...,K.
#
...
# dpps for k = 3,...,K are computed.
#=================================================================================
compute_dpp = function(dpp_2, # Dots per pixel for k = 2
                       K, # Number of different pixel sizes, e.g. 3L
  ...
  if(K < 2 | !is.integer(K)){
    stop('K needs to be an integer greater than or equal to two, e.g. 2L')
  }
  if(any(dpp_2 < 2) | any(!sapply(dpp_2,is.integer))){
    stop('dots per pixel for k = 1 in the x and y direction need to be integers greater than or equal to two, e.g. c(2L,2L)')
  }
  ...
  #----------------------------------------------------------
  # Compute dpp for k = 1,...,K
  #----------------------------------------------------------
  dpp = array(NA, dim = c(K,2), dimnames = list(1:K, c('x','y'))) # Create matrix
  dpp[1,] <- c(1,1) # Always set dpp for k = 1 to one
  dpp[2,] <- dpp_2 # Set dpp for k = 2 (smallest, most resolved, multi-dot pixel size)

  # Set subsequent multi-dot pixel levels, k = 3,..,K
  if(scale == 'linear'){
    if(K > 2){
      for(i in 3:K){
...
# sizes k = 1,...,K depending on the dot dimensions of the dot matrix
#
# Partial pixels are include (i.e.) non-integer pixel counts are rounded up.
#
# dpp specifies dots per pixel for k = 1,...,K pixel sizes in the x and y
...
#
# The dot matrix is expanded such that, for pixel sizes k = 1,...,K, an integer
...
expand_dot_matrix = function(dpp, dot_matrix){
  ...
  to_expand_xy = compute_num_pix_xy(dpp, dot_matrix_dim)[K,]*dpp[K,] - dot_matrix_dim

  # Check expansion does not a single pixel
  if(!all(to_expand_xy < dpp[K,])){stop('Expansion exceeds one x or y pixel')}
  ...
  # Re-order for accordance with membership allocation
...
#
# For each pixel size k = 1,...,K, allocate a pixel membership to each dot
#
...
# dpp specifies dots per pixel for k = 1,...,K pixel sizes in the x and y
# direction dot_matrix_dim specifies the dimensions of the dot matrix in the x
# and y direction. It returns a N by K matrix of pixel memberships where N is
...
allocate_dot_mem = function(dpp, dot_matrix_dim){
  ...
  # Membership vector
  dot_mem = apply(array(1:K, dim = c(K,1)), 1, function(k){
    # For each k:
...
#============================================================
# Function to compute CI widths averaged over over k = K pixels
# and pixelate.
#
# To mitigate differring extent of CLT over different dpp
# and to ensure pixel variation, we determine breaks using the
# distribution of CI widths averaged at the K-th pixel size and
# no smaller.
#============================================================
pixelate_by_CI_width = function(dot_matrix, dot_mem, dpp, no.free_cores){
  ...
  # Extract numbers of pixels for k = 1,...,K
  ...
  # Compute CI widths for pixels where k = K
  writeLines('\nComputing CI widths averaged over dpp for k = K')
  dot_matrix$CI_width_K = NA
  # Sort by dot_mem: allows vectorisation via temp_matrix since all pixels have the same dpp
  sorted_dot_mem_K = sort.int(dot_mem[,K], index.return = T)
  # Create a temporary matrix with pixel entries per column
  temp_matrix = matrix(dot_matrix$CI_width[sorted_dot_mem_K$ix], nrow = prod(dpp[K,]))
  # temp_matrix enables calculation of CI_widths_K using colMeans(), which calls C code directly
  CI_widths_K = colMeans(temp_matrix, na.rm = T)
  # Expand CI_widths_K such that there is one per dot and allocate using indices
  dot_matrix$CI_width_K[sorted_dot_mem_K$ix] = rep(CI_widths_K, each = prod(dpp[K,]))

  # Compute breaks based on quantiles of CI width for k = K
  Breaks = quantile(dot_matrix$CI_width_K,
                    probs=seq(0,1,length.out=K+1),
                    na.rm=T)

  # Allocate pixel types (.bincode is fast for assigning numeric bins)
  dot_matrix$bins = .bincode(dot_matrix$CI_width_K, breaks = Breaks, include.lowest = T)
  ...
  writeLines('\nComputing point estimates for k = 2,...,K')

  #Obsolete code for parallelising over k = 1,...,K
  ...
  # Return Breaks as well as pixelated matrix
  to_return = list(Pix_matrix = dot_matrix, Breaks = Breaks)
