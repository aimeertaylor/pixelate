...
#'
#' @param obs_df Data frame. Contains a row per observation with
...
#' @param scale Character equal to either "linear" or "exponential". Specifies
...
pixelate <- function(obs_df,
  ...
  opp_2 <- compute_opp_2(num_pix_xy_bigk, bigk, scale, scale_factor, obs_df_dim)

  # Observation data frame compatibility check
  opp_min <- compute_opp(2, bigk, scale, scale_factor)
  obs_req <- opp_min[bigk,] * num_pix_xy_bigk
