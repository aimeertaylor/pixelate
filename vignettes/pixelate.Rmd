...
* Should arguements and parameter variables by in code? 
...
The function `pixelate` supports spatially continuous point estimates on any topic providing that they have associated measures of uncertainty. Its output could be plotted using any of the various geographic information systems available. We illustrate its utility using predicted *Plasmodium falciparum* incidence [REF paper] available at the Malaria Atlas Project (MAP) website [map.ox.ac.uk](https://map.ox.ac.uk/), and plot the output of `pixelate` using functions from packages gglpot2 @ggplot2 and sf @sf. 

# Step-by-step quick example


# Step-by-step detailed example

In the example that follows we plot spatial predictions before and after `pixelate` using functions from the gglpot2 package @ggplot2. Before we beging, let us define a function, `plot_spatial_predictions`, to avoid duplicating code over multiple plots. 
...
plot_spatial_predictions = function(spatial_predictions, shape_file, bbox.sf, 
                                    gradient.colors = c("white","red"), # Red since malaria parasites infect red blood cells
                                    na.color = "lightblue", # Blue since most NAs represent water 
